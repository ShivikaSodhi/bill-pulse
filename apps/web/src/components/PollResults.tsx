                  ...
                  {canVote ? (
                    <button onClick={() => onVote!(option.id)} className="w-full text-left group">
                      ...
                      <div className="relative h-9 bg-gray-100 rounded-lg overflow-hidden group-hover:bg-gray-200 transition-colors">
                        <div
                          className="absolute inset-y-0 left-0 bg-brand-500 rounded-lg transition-all duration-500 opacity-30"
                          style={{ width: `${pct}%` }}
                        />
                    ...
                    <div>
                      ...
                      <div className="h-6 bg-gray-100 rounded-lg overflow-hidden">
                        <div
                          className={`h-full rounded-lg transition-all duration-700 ${isLeader ? 'bg-brand-500' : 'bg-brand-300'}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
        ...
        <div className="space-y-3">
          ...
          {textResponses.length > 0 ? (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">{textResponses.length} response{textResponses.length !== 1 ? 's' : ''}</p>
            ...
            isActive && !onTextResponse && (
              <p className="text-sm text-gray-400 text-center py-4">Waiting for responses...</p>
            )
          )}
        </div>
      )}
